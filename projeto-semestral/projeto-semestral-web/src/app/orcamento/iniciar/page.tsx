...
const extrairAno = (dataString: string | null | undefined): string => {
    ...
    try {
        if (dataString.length >= 4 && /^\d{4}/.test(dataString)) { return dataString.substring(0, 4); }
...
export default function IniciarOrcamentoPage() { // << ÚNICA EXPORTAÇÃO DEFAULT
    ...
    const buscarClientes = useCallback(async (e?: FormEvent) => {
        ...
        } catch (err: any) { setError(tratarErroFetch(err, "Busca Cliente")); setClientesEncontrados([]);
    ...
    const fetchVeiculosDoCliente = useCallback(async (cliente: ClienteInfoDTO | null) => {
        ...
        setError(null); setVeiculosDoCliente([]); setVeiculoSelecionado(null);
        const { idCli, idEndereco } = cliente;
        if (!idCli || !idEndereco) { setError("Erro interno: ID do cliente ou endereço inválido."); setIsLoadingVeiculos(false); return; }
    ...
    const resetSelecaoCliente = useCallback(() => {
        console.log("[Reset Cliente] Voltando para busca de cliente.");
        setClienteSelecionado(null); setVeiculosDoCliente([]); setVeiculoSelecionado(null);
        setTermoBuscaCliente(''); setError(null); setClientesEncontrados([]);
    }, []);

    const handleSelecionarVeiculo = useCallback((veiculo: VeiculoResponseDto) => {
        console.log("[Selecionar Veículo] Veículo selecionado:", veiculo);
        setVeiculoSelecionado(veiculo);
    }, []);

    const iniciarGeracaoOrcamento = useCallback(() => {
        console.log("[Iniciar Orçamento] Botão clicado.");
        console.log("[Iniciar Orçamento] Estado atual:", { clienteSelecionado, veiculoSelecionado });
        if (!clienteSelecionado || !veiculoSelecionado) { console.error("[Iniciar Orçamento] Erro: Cliente ou veículo não está selecionado no estado."); setError("Erro interno: Selecione um cliente e um veículo primeiro."); return; }
                    ...
                    <section className="mb-8 p-4 md:p-6 bg-slate-800 rounded-lg shadow-lg max-w-4xl mx-auto border border-slate-600">
                        ...
                        <form onSubmit={buscarClientes} className="flex flex-col sm:flex-row gap-4 items-end">
                            <div className="flex-shrink-0 w-full sm:w-auto">
                                <label htmlFor="tipoBuscaCliente" className="block text-sm font-medium mb-1 text-slate-300 flex items-center gap-1"> <ListFilter size={16} /> Buscar por: </label>
                                <select id="tipoBuscaCliente" value={tipoBuscaCliente} onChange={e => setTipoBuscaCliente(e.target.value as TipoBuscaCliente)} className="w-full sm:w-40 p-2 h-10 rounded bg-slate-700 border border-slate-600 focus:ring-2 focus:ring-sky-500"> <option value="nome">Nome</option> <option value="documento">Documento</option> <option value="idCliente">Código</option> </select>
                            </div>
                            <div className="flex-grow w-full">
                                <label htmlFor="termoBuscaCliente" className="block text-sm font-medium mb-1 text-slate-300 flex items-center gap-1"> {tipoBuscaCliente === 'nome' ? <User size={16} /> : tipoBuscaCliente === 'documento' ? <ScanSearch size={16} /> : <Hash size={16} />} Termo: </label>
                                <input id="termoBuscaCliente" type={tipoBuscaCliente === 'idCliente' ? 'number' : 'text'} value={termoBuscaCliente} onChange={e => setTermoBuscaCliente(e.target.value)} placeholder={getPlaceholderCliente()} required className="w-full p-2 h-10 rounded bg-slate-700 border border-slate-600 focus:ring-2 focus:ring-sky-500" />
                            </div>
                        ...
                        {error && !isBuscandoCliente && (<p className="mt-4 text-sm text-red-400 flex items-center gap-1"><MdErrorOutline/>{error}</p>)}
                        {!isBuscandoCliente && clientesEncontrados.length > 0 && (
                            <div className="mt-6">
                                ...
                                <ul className="space-y-2 max-h-48 overflow-y-auto pr-2">
                                    {clientesEncontrados.map(c => ( <li key={`${c.idCli}-${c.idEndereco}`} onClick={() => handleSelecionarCliente(c)} className="p-3 bg-slate-700 hover:bg-sky-700 cursor-pointer rounded flex justify-between items-center text-sm transition-colors duration-150 shadow-sm" title={`Selecionar Cliente (ID: ${c.idCli})`}> <span className='flex items-center gap-2 text-slate-100'> <User size={16} className='text-slate-400'/> {c.getNomeCompleto?.()} </span> <span className="text-xs text-slate-400 flex items-center gap-1"> <Hash size={14}/>{c.idCli} | <ScanSearch size={14}/> {c.numeroDocumento} </span> </li> ))}
                                </ul>
                            </div>
                        )}
                    </section>
                ...
                {clienteSelecionado && (
                    <section className="mb-6 p-4 md:p-6 bg-slate-800 rounded-lg shadow-lg max-w-4xl mx-auto border border-slate-600">
                        <div className="mb-4 pb-3 border-b border-slate-700 flex flex-wrap justify-between items-center gap-2">
                            <h2 className='text-xl font-semibold text-sky-300 flex items-center gap-2'> <UserCheck size={22}/> Cliente: <span className='text-white font-bold'>{clienteSelecionado.getNomeCompleto?.()}</span> </h2>
                            <button onClick={resetSelecaoCliente} className="text-xs text-sky-400 hover:text-sky-200 underline">Trocar Cliente</button>
                        </div>
                        {error && (<p className="mb-4 text-sm text-red-400 flex items-center gap-1"><MdErrorOutline/>{error}</p>)}
                        {isLoadingVeiculos && ( <div className='flex justify-center items-center py-6'><Loader2 className="h-6 w-6 animate-spin text-sky-400" /><span className='ml-3 text-sky-300'>Carregando veículos...</span></div> )}
                        {!isLoadingVeiculos && !error && (
                            <>
                                <h3 className="text-lg font-semibold text-slate-200 mb-3">2. Selecione o Veículo</h3>
                                {veiculosDoCliente.length === 0 ? ( <p className="text-slate-400 text-sm">Nenhum veículo encontrado para este cliente.</p> ) : (
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
                                        {veiculosDoCliente.map(v => ( <button key={v.id} onClick={() => handleSelecionarVeiculo(v)} className={`p-3 rounded text-left w-full transition-colors duration-150 border ${veiculoSelecionado?.id === v.id ? 'bg-sky-700 border-sky-500 ring-2 ring-sky-400' : 'bg-slate-700 hover:bg-slate-600 border-slate-600'}`}> <p className="font-semibold text-slate-100 flex items-center gap-1"><Car size={16} /> {v.placa} - {v.modelo}</p> <p className="text-xs text-slate-400">{v.montadora} - {v.cor} - Ano: {extrairAno(v.anoFabricacao)}</p> </button> ))}
                                    </div>
                                )}
                            </>
                        )}
                        {clienteSelecionado && veiculoSelecionado && (
                            <div className="mt-6 pt-4 border-t border-slate-700 text-center">
                                ...
                                <button onClick={iniciarGeracaoOrcamento} className="px-6 py-3 bg-green-600 hover:bg-green-700 text-white font-bold rounded-md shadow-lg flex items-center justify-center gap-2 mx-auto">
